...
from .serializers import PlantSerializer, MedicinalPlantSerializer, PlantNameSerializer
...
class PlantNameListView(generics.ListCreateAPIView):
    queryset = MedicinalPlant.objects.all()
    serializer_class = PlantNameSerializer
    filter_backends = (DjangoFilterBackend, filters.SearchFilter)

class PlantNameDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = Plant.objects.all()
    serializer_class = PlantNameSerializer
